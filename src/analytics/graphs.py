...
def home_page():
    df=getdata()
    # add the hardcoded get_stats() here
    subset=df
    subset = subset.filter(f.col("administrative_area_level_1") == 'United States')
    ...
    subset.createOrReplaceTempView("subset")
    subset = spark.sql(
        "SELECT * from subset WHERE deaths = (SELECT MAX(deaths) FROM subset);"
    )
    return subset
